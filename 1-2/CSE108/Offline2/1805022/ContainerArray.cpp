...
public:
    ...
    void setItemAt(Container val,int index) {
        if( index>=allocatedSize ) {
            cout << "Exception: Container Array index out of bound";
        } else {
            if( val.getStoredType() == INTEGER ) {
                int* temp = (int*)val.getItem();
                arrayOfContainers[index].setItem(*temp);
            }
            else if( val.getStoredType() == INT_ARRAY ) {
                int *temp = (int*)val.getItem();
                arrayOfContainers[index].setItem(temp,val.getFirstDim());
            }
            else if( val.getStoredType() == INT_MATRIX ) {
                int **temp = (int **)val.getItem();
                arrayOfContainers[index].setItem(temp,val.getFirstDim(),val.getSecondDim());
            }
            else {
