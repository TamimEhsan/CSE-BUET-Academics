...
bool ScopeTable::insertSymbol(SymbolInfo symbolInfo) {
    ...
    SymbolInfo *newSymbolInfo = new SymbolInfo(symbolInfo);

    if( uid != "1" and symbolInfo.stackOffset == 0 ){
    //if( symbolInfo.stackOffset == -1 ){
        if( symbolInfo.isArray() ){
            newSymbolInfo->stackOffset = stackOffset-2;
            stackOffset-=2*symbolInfo.getSize();
        } else {
            stackOffset-=2;
            newSymbolInfo->stackOffset = stackOffset;
        }
    }
    ...
    cout<<"IInserted in ScopeTable# "<<uid<<" at position "<<place<<", "<<position<<" "<<name<<" "<<type<<" "
        <<symbolInfo.getTypeSpecifier()<<" "<<symbolInfo.getDeclarationList().size()<<" "<<symbolInfo.getParameterList().size()<<" "<<newSymbolInfo->stackOffset<<endl<<endl;
    return true;
}
...
SymbolInfo *ScopeTable::lookUpSymbol(string key) {
    ...
    while( symbolInfoNow != nullptr){
        ...
        if( symbolInfoNow->getName() == key ) {
            ...
            // cout<<"debugg:: "<<symbolInfoNow->stackOffset<<endl;
