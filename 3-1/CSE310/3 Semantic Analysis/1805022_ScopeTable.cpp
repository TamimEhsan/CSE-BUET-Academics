...
bool ScopeTable::insertSymbol(SymbolInfo symbolInfo) {
    ...
    cout<<"Inserted in ScopeTable# "<<uid<<" at position "<<place<<", "<<position<<" "<<name<<" "<<type<<" "
        <<symbolInfo.getTypeSpecifier()<<" "<<symbolInfo.getDeclarationList().size()<<" "<<symbolInfo.getParameterList().size()<<endl<<endl;
    return true;
}
